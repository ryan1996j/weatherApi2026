// const SevereWeatheAlert = () => {
//   return (
//     <>
//       <div className="w-full bg-white/20 backdrop-blur-md border border-white/30 rounded-2xl px-4 py-3">
//         <div className="flex items-start gap-3">
//           {/* Alert icon */}
//           <div className="mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-red-100 text-red-600">
//             <TbAlertTriangleFilled className="size-7" />
//           </div>

//           {/* Alert content */}
//           <div className="min-w-0 flex-1">
//             <h3 className="text-xs font-bold tracking-wide text-red-700">
//               SEVERE WEATHER
//             </h3>

//             <p className="mt-1 text-sm font-medium text-gray-800">
//               Heavy rain warning
//             </p>

//             <p className="mt-1 text-xs text-gray-500">
//               Heavy rainfall is expected in your area.
//             </p>
//           </div>
//         </div>
//       </div>
//     </>
//   );
// };

// export default SevereWeatheAlert;
import { TbAlertTriangleFilled } from "react-icons/tb";

const SevereWeatherAlert = () => {
  return (
    <div className="w-full rounded-2xl shadow-lg border border-white/30 bg-white/20 px-4 py-3 backdrop-blur-md">
      <div className="flex items-start gap-3">
        {/* Alert icon */}
        <div
          className="
            mt-0.5
            flex
            h-9
            w-9
            shrink-0
            items-center
            justify-center
            rounded-full
            bg-[var(--alert-bg)]
            text-[var(--alert-icon)]
          "
        >
          <TbAlertTriangleFilled className="size-7" />
        </div>

        {/* Alert content */}
        <div className="min-w-0 flex-1">
          <h3
            className="
              text-xs
              font-bold
              tracking-wide
              text-[var(--alert-title)]
            "
          >
            SEVERE WEATHER
          </h3>

          <p
            className="
              mt-1
              text-sm
              font-medium
              text-[var(--alert-text)]
            "
          >
            Heavy rain warning
          </p>

          <p
            className="
              mt-1
              text-xs
              text-[var(--alert-muted)]
            "
          >
            Heavy rainfall is expected in your area.
          </p>
        </div>
      </div>
    </div>
  );
};

export default SevereWeatherAlert;
